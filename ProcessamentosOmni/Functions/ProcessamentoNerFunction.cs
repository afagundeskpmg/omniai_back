...
{
    public class ProcessamentoNerFunction
    {      
        [FunctionName("ProcessamentoNerFunction")]
        public void Run([QueueTrigger("processamentoner", Connection = "StorageConnection")] CloudQueueMessage myQueueItem)
        {
            ...
            if (dequeueCount <= 1)
            {
                try
                {
                    using (IUnitOfWorkAplicacao _aplicacao = new UnitOfWorkAplicacao(_configuration))
                    {
                        ...
                        ProcessamentoNer processamento = null;

                        do
                        {
                            processamento = _aplicacao.ProcessamentoNer.SelecionarFirstOrDefault(p => p.Id == model.Id);
                        ...
                        {
                            _aplicacao.ProcessamentoNer.Processar(ref processamento);
