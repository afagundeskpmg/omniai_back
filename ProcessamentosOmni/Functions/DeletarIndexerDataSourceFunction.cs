...
namespace Processamentos.Function
{
    public class DeletarIndexerDataSourceFunction 
    {      

        [FunctionName("deletarindexerdatasource")]
        public void Run([QueueTrigger("DeletarIndexerDataSource", Connection = "StorageConnection")] CloudQueueMessage myQueueItem)
        {
            ...
            if (dequeueCount <= 1)
            {
                try
                {
                    using (IUnitOfWorkAplicacao _aplicacao = new UnitOfWorkAplicacao(_configuration))
                    {
                        ...
                        ProcessamentoIndexer processamento = null;

                        do
                        {
                            processamento = _aplicacao.ProcessamentoIndexer.SelecionarFirstOrDefault(p => p.Id == model.Id);
                        ...
                        {
                            _aplicacao.ProcessamentoIndexer.DeletarIndexerDataSource(ref processamento);
