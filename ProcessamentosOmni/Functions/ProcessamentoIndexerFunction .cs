...
{
    public class ProcessamentoIndexerFunction
    {
        [FunctionName("ProcessamentoIndexerFunction")]
        public void Run([QueueTrigger("processamentoindexer", Connection = "StorageConnection")] CloudQueueMessage myQueueItem)
        {
            ...
            if (dequeueCount <= 1)
            {
                try
                {
                    using (IUnitOfWorkAplicacao _aplicacao = new UnitOfWorkAplicacao(_configuration))
                    {
                        ...
                        {
                            _aplicacao.ProcessamentoIndexer.Processar(ref processamento);
